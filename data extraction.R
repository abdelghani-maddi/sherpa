...
##############
# Extraire les données sans passer par OpenAlex ou DOAJ, en utilisant les filtres de l'API (l'idée c'est de récupérer tout sherpa romeo)
...
# Boucle avec les données avec id
for (i in point_depart:70000) {
  id <- i
  
  tryCatch({
    # Construire l'URL avec l'id actuel.
    url <- paste0("https://v2.sherpa.ac.uk/cgi/retrieve_by_id?item-type=publication&api-key=3F5913F6-677E-11EE-8559-EAD6AA1EFE05&format=Json&identifier=", id)
...
saveRDS(data_list, file = "data_list_all_sherpa.rds") # pour les données DOAJ
